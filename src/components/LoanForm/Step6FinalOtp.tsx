...
export const Step6FinalOtp: React.FC<Step6Props> = ({
  data,
  updateData,
  onSubmit,
  onPrev,
  ...
  const [secondsLeft, setSecondsLeft] = useState(286); // 4:46 as in screenshot
  const [resending, setResending] = useState(false);
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

  const otpArray =
    data.finalOtpCode && data.finalOtpCode.length === 6
      ? data.finalOtpCode
      : ['', '', '', '', '', ''];
  ...
  const handleDigitChange = (index: number, val: string) => {
    ...
    updateData({ finalOtpCode: newOtp });
    ...
    <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
      ...
      <div className="flex items-center justify-between border-b border-[#e1ede7] pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500">
              Confirm your identity to complete the application
            </p>
          </div>
        </div>
        <div className="text-right">
          ...
          <span className="font-urdu text-[11px] text-slate-500" dir="rtl">
            اپنی درخواست مکمل کرنے کے لیے تصدیق کریں
          </span>
        </div>
      </div>
      ...
      <div className="bg-[#eaf6f0] border border-[#c4e6d6] rounded-xl p-4 text-center">
        <p className="text-sm font-semibold text-[#0a4d40] flex items-center justify-center gap-1.5">
          <span>✓</span> A final confirmation code has been sent to your mobile number
        </p>
        <p className="font-urdu text-sm text-[#0c594b] mt-1" dir="rtl">
          آپ کے موبائل نمبر پر حتمی تصدیقی کوڈ بھیج دیا گیا ہے
        </p>
      </div>
      ...
      <div className="pt-5 flex items-center justify-between gap-4 border-t border-[#e2efe9]">
        <button
          ...
          className="px-5 py-2.5 rounded-lg border border-[#c6dfd4] text-slate-600 hover:bg-[#eaf5f0] text-sm font-medium transition-all flex items-center gap-1.5 disabled:opacity-50"
        >
          <span>←</span> Back / واپس
        </button>
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          className="px-6 py-2.5 rounded-lg bg-[#4b7f75] hover:bg-[#3b6b62] active:scale-[0.98] text-white text-sm sm:text-base font-medium transition-all shadow-xs flex items-center gap-2 disabled:opacity-75"
        >
          {isSubmitting ? (
            <>
